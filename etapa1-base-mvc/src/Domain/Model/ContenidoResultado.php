...
use LabResults\Domain\Exception\ValorTextoInvalidoException;

/**
 * Value Object inmutable con el CONTENIDO de un resultado
 * (tipo + valor + unidad). Se usa tanto para la captura inicial
 * como para cada corrección versionada.
 *
...
final readonly class ContenidoResultado
{
    ...
    public static function crear(
        ...
        $tipo = ResultType::tryFrom($tipoLimpio)
        ...
        return match ($tipo) {
            ResultType::Numerico => new self($tipo, self::parsearNumero($valorNumericoRaw), null, $unidad),
            ResultType::Texto => new self($tipo, null, self::parsearTexto($valorTextoRaw), self::rechazarUnidadEnTexto($unidad)),
        };
    }
    ...
    /**
     * Rehidratación desde persistencia: los datos ya fueron validados
     * al momento de guardarse; aquí no se vuelven a validar.
     */
    ...
    private static function parsearNumero(?string $raw): float
    {
        ...
        if ($limpio === null || !is_numeric($limpio)) {
            throw new ValorNumericoInvalidoException(
                'El valor del resultado debe ser un número válido.'
            );
        }
        ...
        if (!is_finite($numero)) {
            throw new ValorNumericoInvalidoException(
                'El valor numérico no es finito y por lo tanto es inválido.'
            );
        }
    ...
    private static function parsearTexto(?string $raw): string
    {
        ...
        if ($limpio === '') {
            throw new ValorTextoInvalidoException(
                'El valor textual del resultado es obligatorio.'
            );
        }
    ...
    private static function rechazarUnidadEnTexto(?string $unidad): ?string
    {
        if ($unidad !== null && $unidad !== '') {
            throw new UnidadInvalidaException(
                'Los resultados de tipo TEXTO no llevan unidad de medida.'
            );
        }
